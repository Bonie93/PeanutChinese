const HSK1 = [
  { hanzi: "你好", pinyin: "nǐ hǎo", meaning: "Xin chào" },
  { hanzi: "谢谢", pinyin: "xiè xie", meaning: "Cảm ơn" },
  { hanzi: "再见", pinyin: "zài jiàn", meaning: "Tạm biệt" },
  { hanzi: "是", pinyin: "shì", meaning: "Là" },
  { hanzi: "我", pinyin: "wǒ", meaning: "Tôi" },
  { hanzi: "你", pinyin: "nǐ", meaning: "Bạn" },
  { hanzi: "他", pinyin: "tā", meaning: "Anh ấy" },
  { hanzi: "她", pinyin: "tā", meaning: "Cô ấy" },
  { hanzi: "它", pinyin: "tā", meaning: "Nó" },
  { hanzi: "我们", pinyin: "wǒ men", meaning: "Chúng tôi" },
  { hanzi: "你们", pinyin: "nǐ men", meaning: "Các bạn" },
  { hanzi: "他们", pinyin: "tā men", meaning: "Họ" },
  { hanzi: "这", pinyin: "zhè", meaning: "Đây, này" },
  { hanzi: "那", pinyin: "nà", meaning: "Kia, đó" },
  { hanzi: "哪", pinyin: "nǎ", meaning: "Nào, đâu" },
  { hanzi: "谁", pinyin: "shéi", meaning: "Ai" },
  { hanzi: "什么", pinyin: "shén me", meaning: "Cái gì" },
  { hanzi: "怎么", pinyin: "zěn me", meaning: "Thế nào" },
  { hanzi: "为什么", pinyin: "wèi shén me", meaning: "Tại sao" },
  { hanzi: "几", pinyin: "jǐ", meaning: "Mấy" },
  { hanzi: "多少", pinyin: "duō shǎo", meaning: "Bao nhiêu" },
  { hanzi: "有", pinyin: "yǒu", meaning: "Có" },
  { hanzi: "没有", pinyin: "méi yǒu", meaning: "Không có" },
  { hanzi: "在", pinyin: "zài", meaning: "Ở, tại" },
  { hanzi: "不", pinyin: "bù", meaning: "Không" },
  { hanzi: "很", pinyin: "hěn", meaning: "Rất" },
  { hanzi: "也", pinyin: "yě", meaning: "Cũng" },
  { hanzi: "都", pinyin: "dōu", meaning: "Đều" },
  { hanzi: "和", pinyin: "hé", meaning: "Và" },
  { hanzi: "的", pinyin: "de", meaning: "Trợ từ sở hữu" }
  { hanzi: "吗", pinyin: "ma", meaning: "Trợ từ nghi vấn" },
  { hanzi: "呢", pinyin: "ne", meaning: "Thì sao, còn..." },
  { hanzi: "吧", pinyin: "ba", meaning: "Nhé, nhé nào" },
  { hanzi: "啊", pinyin: "a", meaning: "À, a" },
  { hanzi: "请", pinyin: "qǐng", meaning: "Mời; xin vui lòng" },
  { hanzi: "对不起", pinyin: "duì bu qǐ", meaning: "Xin lỗi" },
  { hanzi: "没关系", pinyin: "méi guān xi", meaning: "Không sao" },
  { hanzi: "不用谢", pinyin: "bú yòng xiè", meaning: "Không có gì" },
  { hanzi: "认识", pinyin: "rèn shi", meaning: "Quen, biết" },
  { hanzi: "叫", pinyin: "jiào", meaning: "Tên là; gọi" },
  { hanzi: "名字", pinyin: "míng zi", meaning: "Tên" },
  { hanzi: "姓", pinyin: "xìng", meaning: "Họ" },
  { hanzi: "老师", pinyin: "lǎo shī", meaning: "Giáo viên" },
  { hanzi: "学生", pinyin: "xué sheng", meaning: "Học sinh" },
  { hanzi: "朋友", pinyin: "péng you", meaning: "Bạn bè" },
  { hanzi: "同学", pinyin: "tóng xué", meaning: "Bạn học" },
  { hanzi: "爸爸", pinyin: "bà ba", meaning: "Bố" },
  { hanzi: "妈妈", pinyin: "mā ma", meaning: "Mẹ" },
  { hanzi: "儿子", pinyin: "ér zi", meaning: "Con trai" },
  { hanzi: "女儿", pinyin: "nǚ ér", meaning: "Con gái" },
  { hanzi: "家", pinyin: "jiā", meaning: "Nhà; gia đình" },
  { hanzi: "学校", pinyin: "xué xiào", meaning: "Trường học" },
  { hanzi: "中国", pinyin: "Zhōng guó", meaning: "Trung Quốc" },
  { hanzi: "人", pinyin: "rén", meaning: "Người" },
  { hanzi: "中国人", pinyin: "Zhōng guó rén", meaning: "Người Trung Quốc" },
  { hanzi: "天气", pinyin: "tiān qì", meaning: "Thời tiết" },
  { hanzi: "今天", pinyin: "jīn tiān", meaning: "Hôm nay" },
  { hanzi: "明天", pinyin: "míng tiān", meaning: "Ngày mai" },
  { hanzi: "昨天", pinyin: "zuó tiān", meaning: "Hôm qua" },
  { hanzi: "现在", pinyin: "xiàn zài", meaning: "Bây giờ" },
  { hanzi: "时候", pinyin: "shí hou", meaning: "Lúc, thời điểm" },
  { hanzi: "点", pinyin: "diǎn", meaning: "Giờ; chấm; điểm" },
  { hanzi: "分钟", pinyin: "fēn zhōng", meaning: "Phút" },
  { hanzi: "小时", pinyin: "xiǎo shí", meaning: "Giờ (đơn vị thời gian)" },
  { hanzi: "年", pinyin: "nián", meaning: "Năm" },
  { hanzi: "月", pinyin: "yuè", meaning: "Tháng; mặt trăng" },
  { hanzi: "日", pinyin: "rì", meaning: "Ngày; mặt trời" },
  { hanzi: "号", pinyin: "hào", meaning: "Ngày (trong tháng); số" },
  { hanzi: "星期", pinyin: "xīng qī", meaning: "Tuần" },
  { hanzi: "星期一", pinyin: "xīng qī yī", meaning: "Thứ Hai" },
  { hanzi: "星期二", pinyin: "xīng qī èr", meaning: "Thứ Ba" },
  { hanzi: "星期三", pinyin: "xīng qī sān", meaning: "Thứ Tư" },
  { hanzi: "星期四", pinyin: "xīng qī sì", meaning: "Thứ Năm" },
  { hanzi: "星期五", pinyin: "xīng qī wǔ", meaning: "Thứ Sáu" },
  { hanzi: "星期六", pinyin: "xīng qī liù", meaning: "Thứ Bảy" },
  { hanzi: "星期天", pinyin: "xīng qī tiān", meaning: "Chủ Nhật" },
  { hanzi: "今天上午", pinyin: "jīn tiān shàng wǔ", meaning: "Sáng nay" },
  { hanzi: "今天下午", pinyin: "jīn tiān xià wǔ", meaning: "Chiều nay" },
  { hanzi: "今天晚上", pinyin: "jīn tiān wǎn shang", meaning: "Tối nay" },
  { hanzi: "早上", pinyin: "zǎo shang", meaning: "Buổi sáng" }
  { hanzi: "爱", pinyin: "ài", meaning: "Yêu; thích" },
  { hanzi: "爱好", pinyin: "ài hào", meaning: "Sở thích" },
  { hanzi: "八", pinyin: "bā", meaning: "Số tám" },
  { hanzi: "爸爸", pinyin: "bà ba", meaning: "Bố" },
  { hanzi: "白", pinyin: "bái", meaning: "Màu trắng" },
  { hanzi: "白天", pinyin: "bái tiān", meaning: "Ban ngày" },
  { hanzi: "百", pinyin: "bǎi", meaning: "Một trăm" },
  { hanzi: "班", pinyin: "bān", meaning: "Lớp học; ca làm" },
  { hanzi: "半", pinyin: "bàn", meaning: "Một nửa" },
  { hanzi: "半年", pinyin: "bàn nián", meaning: "Nửa năm" },
  { hanzi: "半天", pinyin: "bàn tiān", meaning: "Nửa ngày" },
  { hanzi: "帮", pinyin: "bāng", meaning: "Giúp" },
  { hanzi: "帮忙", pinyin: "bāng máng", meaning: "Giúp đỡ" },
  { hanzi: "包", pinyin: "bāo", meaning: "Túi; gói" },
  { hanzi: "包子", pinyin: "bāo zi", meaning: "Bánh bao" },
  { hanzi: "杯", pinyin: "bēi", meaning: "Cái cốc (lượng từ)" },
  { hanzi: "杯子", pinyin: "bēi zi", meaning: "Cái cốc" },
  { hanzi: "北", pinyin: "běi", meaning: "Phía bắc" },
  { hanzi: "北边", pinyin: "běi biān", meaning: "Phía bắc" },
  { hanzi: "北京", pinyin: "Běi jīng", meaning: "Bắc Kinh" }
  { hanzi: "本", pinyin: "běn", meaning: "Quyển (lượng từ cho sách, vở)" },
  { hanzi: "本子", pinyin: "běn zi", meaning: "Quyển vở" },
  { hanzi: "比", pinyin: "bǐ", meaning: "So với" },
  { hanzi: "别", pinyin: "bié", meaning: "Đừng" },
  { hanzi: "别的", pinyin: "bié de", meaning: "Khác" },
  { hanzi: "别人", pinyin: "bié rén", meaning: "Người khác" },
  { hanzi: "病", pinyin: "bìng", meaning: "Bệnh; bị bệnh" },
  { hanzi: "病人", pinyin: "bìng rén", meaning: "Bệnh nhân" },
  { hanzi: "不大", pinyin: "bú dà", meaning: "Không lớn lắm" },
  { hanzi: "不对", pinyin: "bú duì", meaning: "Không đúng" },
  { hanzi: "不客气", pinyin: "bú kè qi", meaning: "Không có gì (đáp lại lời cảm ơn)" },
  { hanzi: "不用", pinyin: "bú yòng", meaning: "Không cần" },
  { hanzi: "菜", pinyin: "cài", meaning: "Món ăn; rau" },
  { hanzi: "差", pinyin: "chà", meaning: "Kém; thiếu" },
  { hanzi: "常", pinyin: "cháng", meaning: "Thường" },
  { hanzi: "常常", pinyin: "cháng cháng", meaning: "Thường xuyên" },
  { hanzi: "唱", pinyin: "chàng", meaning: "Hát" },
  { hanzi: "唱歌", pinyin: "chàng gē", meaning: "Hát" },
  { hanzi: "车", pinyin: "chē", meaning: "Xe" },
  { hanzi: "车票", pinyin: "chē piào", meaning: "Vé xe" }
  { hanzi: "车上", pinyin: "chē shàng", meaning: "Trên xe" },
  { hanzi: "车站", pinyin: "chē zhàn", meaning: "Nhà ga; bến xe" },
  { hanzi: "吃饭", pinyin: "chī fàn", meaning: "Ăn cơm" },
  { hanzi: "出", pinyin: "chū", meaning: "Ra; đi ra" },
  { hanzi: "出来", pinyin: "chū lái", meaning: "Đi ra; bước ra" },
  { hanzi: "出去", pinyin: "chū qù", meaning: "Đi ra ngoài" },
  { hanzi: "穿", pinyin: "chuān", meaning: "Mặc; đi (giày)" },
  { hanzi: "床", pinyin: "chuáng", meaning: "Giường" },
  { hanzi: "次", pinyin: "cì", meaning: "Lần (lượng từ)" },
  { hanzi: "从", pinyin: "cóng", meaning: "Từ" },
  { hanzi: "错", pinyin: "cuò", meaning: "Sai" },
  { hanzi: "打", pinyin: "dǎ", meaning: "Đánh; làm; gọi" },
  { hanzi: "打车", pinyin: "dǎ chē", meaning: "Đi taxi" },
  { hanzi: "打电话", pinyin: "dǎ diàn huà", meaning: "Gọi điện thoại" },
  { hanzi: "打开", pinyin: "dǎ kāi", meaning: "Mở" },
  { hanzi: "打球", pinyin: "dǎ qiú", meaning: "Chơi bóng" },
  { hanzi: "大", pinyin: "dà", meaning: "To; lớn" },
  { hanzi: "大学", pinyin: "dà xué", meaning: "Đại học" },
  { hanzi: "大学生", pinyin: "dà xué shēng", meaning: "Sinh viên đại học" },
  { hanzi: "到", pinyin: "dào", meaning: "Đến" }
  { hanzi: "得到", pinyin: "dé dào", meaning: "Nhận được; có được" },
  { hanzi: "地", pinyin: "de", meaning: "Trợ từ đứng trước động từ" },
  { hanzi: "等", pinyin: "děng", meaning: "Đợi" },
  { hanzi: "地点", pinyin: "dì diǎn", meaning: "Địa điểm" },
  { hanzi: "地方", pinyin: "dì fang", meaning: "Địa điểm; nơi chốn" },
  { hanzi: "地上", pinyin: "dì shàng", meaning: "Trên mặt đất" },
  { hanzi: "地图", pinyin: "dì tú", meaning: "Bản đồ" },
  { hanzi: "弟弟", pinyin: "dì di", meaning: "Em trai" },
  { hanzi: "第", pinyin: "dì", meaning: "Tiền tố chỉ số thứ tự (thứ...)" },
  { hanzi: "电", pinyin: "diàn", meaning: "Điện" },
  { hanzi: "电话", pinyin: "diàn huà", meaning: "Điện thoại" },
  { hanzi: "电脑", pinyin: "diàn nǎo", meaning: "Máy tính" },
  { hanzi: "电视", pinyin: "diàn shì", meaning: "Ti vi" },
  { hanzi: "电视机", pinyin: "diàn shì jī", meaning: "Máy thu hình" },
  { hanzi: "电影", pinyin: "diàn yǐng", meaning: "Phim" },
  { hanzi: "电影院", pinyin: "diàn yǐng yuàn", meaning: "Rạp chiếu phim" },
  { hanzi: "东", pinyin: "dōng", meaning: "Phía đông" },
  { hanzi: "东边", pinyin: "dōng biān", meaning: "Phía đông" },
  { hanzi: "东西", pinyin: "dōng xi", meaning: "Đồ vật; đồ đạc" },
  { hanzi: "动", pinyin: "dòng", meaning: "Động; di chuyển" }
  { hanzi: "读", pinyin: "dú", meaning: "Đọc" },
  { hanzi: "读书", pinyin: "dú shū", meaning: "Đọc sách; học" },
  { hanzi: "对", pinyin: "duì", meaning: "Đúng; đối với" },
  { hanzi: "对不起", pinyin: "duì bu qǐ", meaning: "Xin lỗi" },
  { hanzi: "多", pinyin: "duō", meaning: "Nhiều" },
  { hanzi: "多少", pinyin: "duō shǎo", meaning: "Bao nhiêu" },
  { hanzi: "饿", pinyin: "è", meaning: "Đói" },
  { hanzi: "儿子", pinyin: "ér zi", meaning: "Con trai" },
  { hanzi: "二", pinyin: "èr", meaning: "Số hai" },
  { hanzi: "饭", pinyin: "fàn", meaning: "Cơm; bữa ăn" },
  { hanzi: "饭店", pinyin: "fàn diàn", meaning: "Nhà hàng; khách sạn" },
  { hanzi: "房间", pinyin: "fáng jiān", meaning: "Căn phòng" },
  { hanzi: "房子", pinyin: "fáng zi", meaning: "Ngôi nhà" },
  { hanzi: "放", pinyin: "fàng", meaning: "Đặt; để; thả" },
  { hanzi: "放假", pinyin: "fàng jià", meaning: "Nghỉ lễ; nghỉ phép" },
  { hanzi: "放学", pinyin: "fàng xué", meaning: "Tan học" },
  { hanzi: "飞", pinyin: "fēi", meaning: "Bay" },
  { hanzi: "飞机", pinyin: "fēi jī", meaning: "Máy bay" },
  { hanzi: "非常", pinyin: "fēi cháng", meaning: "Rất; vô cùng" },
  { hanzi: "分", pinyin: "fēn", meaning: "Phút; chia" },
];

window.HSK1 = HSK1;
